...
def print_all_quality_index(q1, q2, q3, output):
    ...
    ax0.text(0.22, 0.75, 'Global quality score', fontsize = 14, fontweight='bold', transform=plt.gcf().transFigure)
    ax1 = fig.add_subplot(gs[0, 1], projection='polar')
    ax1 = plot_quality_index(round(np.mean(q1)), ax1, scale = 5)
    ...
    ax2 = plot_quality_index(round(np.mean(q2)), ax2, scale = 5)
    ...
    ax3 = plot_quality_index(round(np.mean(q3)), ax3, scale = 5)
